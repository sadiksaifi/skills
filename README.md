...
Opinionated [Agent Skills](https://agentskills.io) for building software the way strong teams actually work: scope it, spec it, plan it, slice it, forge it, ship it, resolve it.
...
```
Vague idea:             snap-scope → snap-prd → snap-slice → snap-forge → snap-ship
Have PRD, need slices:  snap-slice → snap-forge → snap-ship
Have PRD, need plan:    snap-plan → snap-forge → snap-ship
Have issue, ready:      snap-forge → snap-ship
