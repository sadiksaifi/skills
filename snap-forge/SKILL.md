...
description: >
  ...
  red-green cycles, or execution from a scoped PRD/issue/slice. Trigger on
...
1. `Orient:` Resolve PRD, issue, slice, or direct task from session context first. If a PRD/issue/slice ref is provided and needed body/comments are not already in session context, fetch them. Extract refs/URLs from available context. Read only missing parent PRD/epic, breakdown comments, or referenced issues/PRs/discussions/docs that affect scope, acceptance, blockers, or implementation constraints. Recurse as needed; normalize + dedupe canonical refs; keep inaccessible/conflicting context as explicit blockers or unknowns. Do not refetch artifacts already available in session unless freshness is required or the user asks. Read enough linked artifacts to reconstruct the full epic context before selecting the behavior. Read code, tests, CI, existing patterns. Detect test stack before writing.
2. `Scope ledger:` Extract acceptance items, explicit non-goals, blockers, and linked-slice boundaries. Keep a visible Done / In progress / Not done ledger. If user asked for a PRD/issue/slice, the selected scope is the whole artifact unless they explicitly narrowed it.
...
Respect slice blockers and explicit user scope. Finish with full-suite verify + terse execution summary against the scope ledger.
...
Refs: `references/tests.md`, `references/refactoring.md`, `references/deep-modules.md`, `references/interface-design.md`, `references/mocking.md`
