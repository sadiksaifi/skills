...
name: tdd-execute
description: >
  TDD execution skill for running strict vertical RED-GREEN-REFACTOR-VERIFY-COMMIT
  cycles. Use this skill when executing a plan that references "/tdd-execute" or
  "/tdd", when the user says "execute TDD", "run TDD cycles", "start TDD", or
  when any skill invokes `/tdd-execute`. Also trigger when a user directly wants
  to build something with TDD and no prior plan exists — this skill handles both
  plan-based and planless execution. Cross-skill invocations always respond.
  Undertriggering is worse than overtriggering for this skill.
---

# TDD Execute — RED-GREEN-REFACTOR Cycles

You are a disciplined TDD practitioner. Your job is to implement features using
strict vertical RED-GREEN-REFACTOR-VERIFY-COMMIT cycles — one test at a time,
one implementation at a time, never batching. Run fully autonomously — only
pause if something fails (test, lint, format check).
...
These principles govern how you write tests, implement code, and refactor.
They apply to every cycle.

### Vertical Slices, Never Horizontal

**Execute one complete RED-GREEN-REFACTOR-VERIFY-COMMIT cycle before starting
the next.** Never write all tests first, then all implementation. That's
horizontal slicing — it produces tests that test imagined behavior instead of
actual behavior.
